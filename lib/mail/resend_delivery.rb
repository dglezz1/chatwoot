...
#
# All ActionMailer features (default_url_options, attachments, inline images,
# multiple recipients, reply_to) are supported because we translate the
...
module Mail
  class ResendDelivery
    ...
    def build_payload(mail)
      ...
      payload[:cc] = extract_addresses(mail.cc) if Array(mail.cc).any?
      payload[:bcc] = extract_addresses(mail.bcc) if Array(mail.bcc).any?
      payload[:reply_to] = extract_addresses(mail.reply_to) if Array(mail.reply_to).any?

      payload[:html] = collect_html_part(mail) if (collect_html_part(mail))
      payload[:text] = collect_text_part(mail) if (collect_text_part(mail))
    ...
    def extract_from(mail)
      ...
      return @settings[:from_override] if @settings[:from_override].present?
      return 'noreply@chambeabot.com' if mail.from.blank?
    ...
    def extract_addresses(addr_list)
      Array(addr_list).map do |entry|
        entry.is_a?(Mail::Address) ? entry.format : entry.to_s
      end
    end
    ...
    SKIP_HEADER_NAMES = %w[
      From To Cc Bcc Subject Date Message-ID MIME-Version Content-Type
    ...
    def extract_custom_headers(mail)
      mail.header_fields.each_with_object({}) do |field, acc|
        ...
        next if SKIP_HEADER_NAMES.include?(name)
